...
int rf_feast(int choice1, struct gameState *state, int handPos){
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      updateCoins(currentPlayer, state, 4);  //update amount of coins to 4 instead of 5
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (card==smithy){
    rf_smithy(state, handPos);
  }
  else if (card==adventurer){
    rf_adventurer(state, handPos);
  }
  else if (card==feast){
    rf_feast(choice1, state, handPos);
  }
  else if (card==village){
    rf_village(state, handPos);
  }
  else if (card==remodel){
    rf_remodel(choice1, choice2, state, handPos);
  }
  else{
